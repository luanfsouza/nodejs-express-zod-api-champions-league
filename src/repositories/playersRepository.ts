...
export const findPlayerById = async (
  ...
  const player = database.find((value) => value.id == id);
  return player;
};

export const CreatePlayer = async (data: PlayerModel): Promise<boolean> => {
  database.push(data);
  return true;
};
...
export const updatePlayerById = async (
  ...
  const index = database.findIndex((value) => value.id == id);
  const playerUpdated: StatisticModel = data;
  if (index !== -1) {
    database[index] = { ...database[index], ...playerUpdated };
